...
export default function DataEntryFormComponent({ 
  ...
  const totalSitePages = Math.ceil(siteFields.length / SITES_PER_PAGE);
  const paginatedSiteFields = useMemo(() => {
    const startIndex = (currentSitePage - 1) * SITES_PER_PAGE;
    const endIndex = startIndex + SITES_PER_PAGE;
    return siteFields.slice(startIndex, endIndex);
  }, [siteFields, currentSitePage]);
                ...
                <AccordionContent className="p-6 pt-0">
                    <div className="border-t pt-6 space-y-4">
                        <Accordion type="multiple" className="w-full space-y-2">
                            ...
                            const purpose = watchedSiteDetails[absoluteIndex]?.purpose;
                            ...
                            const workStatus = watchedSiteDetails[absoluteIndex]?.workStatus;
                              ...
                              <>
                                ...
                                <div className="grid md:grid-cols-4 gap-6">
                                    {isRigAccessibilityRequired && (
                                        <FormField control={form.control} name={`siteDetails.${absoluteIndex}.accessibleRig`} render={({ field }) => (
                                            <FormItem>
                                    ...
                                    <FormField control={form.control} name={`siteDetails.${absoluteIndex}.estimateAmount`} render={({ field }) => (<FormItem><FormLabel>Estimate (₹)</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isEditor} /></FormControl><FormMessage/></FormItem>)}/>
                                    <FormField control={form.control} name={`siteDetails.${absoluteIndex}.remittedAmount`} render={({ field }) => (<FormItem><FormLabel>Remitted Amount (₹)</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isEditor} /></FormControl><FormMessage/></FormItem>)}/>
                                    <FormField control={form.control} name={`siteDetails.${absoluteIndex}.tsAmount`} render={({ field }) => (<FormItem><FormLabel>TS Amount (₹)</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isEditor} /></FormControl><FormMessage/></FormItem>)}/>
                                    <FormField control={form.control} name={`siteDetails.${absoluteIndex}.tenderNo`} render={({ field }) => (<FormItem><FormLabel>Tender No.</FormLabel><FormControl><Input {...field} readOnly={!isEditor} /></FormControl><FormMessage/></FormItem>)}/>
                                    <FormField control={form.control} name={`siteDetails.${absoluteIndex}.contractorName`} render={({ field }) => (<FormItem><FormLabel>Contractor Name</FormLabel><FormControl><Input {...field} readOnly={!isEditor} /></FormControl><FormMessage/></FormItem>)}/>
                                    <FormField
                                        key={item.id}
                                        control={form.control}
                                        name={`siteDetails.${absoluteIndex}.supervisorUid`}
                                        render={({ field }) => (
                                        <FormItem>
                                        ...
                                        {isEditor ? (
                                            ...
                                            onValueChange={(value) => {
                                                if (value === '_unassign_') {
                                                field.onChange(null);
                                                formSetValue(`siteDetails.${absoluteIndex}.supervisorName`, null);
                                                } else {
                                                field.onChange(value);
                                                const selectedStaff = supervisorList.find(s => s.uid === value);
                                                formSetValue(`siteDetails.${absoluteIndex}.supervisorName`, selectedStaff?.name || null);
                                                }
                                            }}
                                            value={field.value || ''}>
                                            ...
                                            <Input
                                            readOnly
                                            value={formGetValues(`siteDetails.${absoluteIndex}.supervisorName`) || "Not Assigned"}
                            ...
                            return (
                                <AccordionItem value={`site-${absoluteIndex}`} key={item.id} className="border bg-card rounded-lg shadow-sm">
                                    <AccordionTrigger className="p-4 hover:no-underline">
                                        <div className="flex flex-1 items-center justify-between">
                                            <span className="text-lg font-semibold text-primary text-left">
                                                Site #{absoluteIndex + 1}
                                                {watchedSiteDetails[absoluteIndex]?.nameOfSite ? `: ${watchedSiteDetails[absoluteIndex].nameOfSite}` : ''}
                                                {watchedSiteDetails[absoluteIndex]?.purpose ? ` (${watchedSiteDetails[absoluteIndex].purpose})` : ''}
                                            </span>
                                            {isEditor && siteFields.length > 1 && 
                                                <div role="button" aria-label={`Remove Site #${absoluteIndex + 1}`} className="p-2 rounded-full hover:bg-destructive/10" onClick={(e) => { e.stopPropagation(); removeSite(absoluteIndex); }}>
                                    ...
                                    <AccordionContent className="p-6 pt-0">
                                        <div className="border-t pt-6 space-y-6">
                                            <div className="grid md:grid-cols-2 gap-6">
                                                <FormField control={form.control} name={`siteDetails.${absoluteIndex}.nameOfSite`} render={({ field }) => (<FormItem><FormLabel>Name of Site <span className="text-destructive">*</span></FormLabel><FormControl><Input {...field} readOnly={!isEditor} /></FormControl><FormMessage/></FormItem>)}/>
                                                <FormField control={form.control} name={`siteDetails.${absoluteIndex}.purpose`} render={({ field }) => (<FormItem><FormLabel>Purpose <span className="text-destructive">*</span></FormLabel><Select onValueChange={field.onChange} value={field.value} disabled={!isEditor}><FormControl><SelectTrigger><SelectValue placeholder="Select Purpose"/></SelectTrigger></FormControl><SelectContent>{sitePurposeOptions.map(o=><SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select><FormMessage/></FormItem>)}/>
                                            </div>
                                            <div className="grid md:grid-cols-2 gap-6">
                                                <FormField control={form.control} name={`siteDetails.${absoluteIndex}.latitude`} render={({ field }) => (<FormItem><FormLabel>Latitude</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/>
                                                <FormField control={form.control} name={`siteDetails.${absoluteIndex}.longitude`} render={({ field }) => (<FormItem><FormLabel>Longitude</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/>
                                            </div>
                                            ...
                                            {isWellPurpose && (
                                                <>
                                                    ...
                                                    {purpose === 'BWC' && ( <div className="grid md:grid-cols-3 gap-6"> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedDiameter`} render={({ field }) => (<FormItem><FormLabel>Diameter (mm)</FormLabel><Select onValueChange={(value) => field.onChange(value === '_clear_' ? undefined : value)} value={field.value} disabled={!isSiteEditable}><FormControl><SelectTrigger><SelectValue placeholder="Select Diameter" /></SelectTrigger></FormControl><SelectContent><SelectItem value="_clear_">-- Clear Selection --</SelectItem>{siteDiameterOptions.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedTD`} render={({ field }) => (<FormItem><FormLabel>TD (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedOB`} render={({ field }) => (<FormItem><FormLabel>OB (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedCasingPipe`} render={({ field }) => (<FormItem><FormLabel>Casing Pipe (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyLocation`} render={({ field }) => (<FormItem><FormLabel>Location</FormLabel><FormControl><Textarea {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRemarks`} render={({ field }) => (<FormItem><FormLabel>Survey Remarks</FormLabel><FormControl><Textarea {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> </div> )}
                                                    {purpose === 'TWC' && ( <div className="grid md:grid-cols-3 gap-6"> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedDiameter`} render={({ field }) => (<FormItem><FormLabel>Diameter (mm)</FormLabel><Select onValueChange={(value) => field.onChange(value === '_clear_' ? undefined : value)} value={field.value} disabled={!isSiteEditable}><FormControl><SelectTrigger><SelectValue placeholder="Select Diameter" /></SelectTrigger></FormControl><SelectContent><SelectItem value="_clear_">-- Clear Selection --</SelectItem>{siteDiameterOptions.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedTD`} render={({ field }) => (<FormItem><FormLabel>TD (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedPlainPipe`} render={({ field }) => (<FormItem><FormLabel>Plain Pipe (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedSlottedPipe`} render={({ field }) => (<FormItem><FormLabel>Slotted Pipe (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedMsCasingPipe`} render={({ field }) => (<FormItem><FormLabel>MS Casing (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyLocation`} render={({ field }) => (<FormItem><FormLabel>Location</FormLabel><FormControl><Textarea {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRemarks`} render={({ field }) => (<FormItem><FormLabel>Survey Remarks</FormLabel><FormControl><Textarea {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> </div> )}
                                                    {purpose === 'FPW' && ( <div className="grid md:grid-cols-3 gap-6"> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedDiameter`} render={({ field }) => (<FormItem><FormLabel>Diameter (mm)</FormLabel><Select onValueChange={(value) => field.onChange(value === '_clear_' ? undefined : value)} value={field.value} disabled={!isSiteEditable}><FormControl><SelectTrigger><SelectValue placeholder="Select Diameter" /></SelectTrigger></FormControl><SelectContent><SelectItem value="_clear_">-- Clear Selection --</SelectItem>{siteDiameterOptions.map(o => <SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedTD`} render={({ field }) => (<FormItem><FormLabel>TD (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRecommendedCasingPipe`} render={({ field }) => (<FormItem><FormLabel>Casing Pipe (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyLocation`} render={({ field }) => (<FormItem><FormLabel>Location</FormLabel><FormControl><Textarea {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyRemarks`} render={({ field }) => (<FormItem><FormLabel>Survey Remarks</FormLabel><FormControl><Textarea {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> </div> )}

                                                    {workImplementationFields}
                                                    
                                                    <Separator className="my-4" />
                                                    <h4 className="text-md font-medium text-primary mb-2">Drilling Details (Actuals)</h4>
                                                    {purpose === 'BWC' && ( <div className="grid md:grid-cols-3 gap-6"> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.diameter`} render={({ field }) => (<FormItem><FormLabel>Diameter (mm){isDiameterRequired && <span className="text-destructive">*</span>}</FormLabel><Select onValueChange={(value) => field.onChange(value === '_clear_' ? undefined : value)} value={field.value} disabled={!isSiteEditable}><FormControl><SelectTrigger><SelectValue placeholder="Select Diameter"/></SelectTrigger></FormControl><SelectContent><SelectItem value="_clear_">-- Clear Selection --</SelectItem>{siteDiameterOptions.map(o=><SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.totalDepth`} render={({ field }) => (<FormItem><FormLabel>TD (m)</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyOB`} render={({ field }) => (<FormItem><FormLabel>OB (m)</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.casingPipeUsed`} render={({ field }) => (<FormItem><FormLabel>Casing Pipe (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.innerCasingPipe`} render={({ field }) => (<FormItem><FormLabel>Inner Casing (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.outerCasingPipe`} render={({ field }) => (<FormItem><FormLabel>Outer Casing (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.yieldDischarge`} render={({ field }) => (<FormItem><FormLabel>Discharge (LPH)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.zoneDetails`} render={({ field }) => (<FormItem><FormLabel>Zone Details (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.waterLevel`} render={({ field }) => (<FormItem><FormLabel>Water Level (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.drillingRemarks`} render={({ field }) => (<FormItem className="md:col-span-3"><FormLabel>Drilling Remarks</FormLabel><FormControl><Textarea {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage /></FormItem>)}/> </div> )}
                                                    {purpose === 'TWC' && ( <div className="grid md:grid-cols-3 gap-6"> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.diameter`} render={({ field }) => (<FormItem><FormLabel>Diameter (mm){isDiameterRequired && <span className="text-destructive">*</span>}</FormLabel><Select onValueChange={(value) => field.onChange(value === '_clear_' ? undefined : value)} value={field.value} disabled={!isSiteEditable}><FormControl><SelectTrigger><SelectValue placeholder="Select Diameter"/></SelectTrigger></FormControl><SelectContent><SelectItem value="_clear_">-- Clear Selection --</SelectItem>{siteDiameterOptions.map(o=><SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.totalDepth`} render={({ field }) => (<FormItem><FormLabel>TD (m)</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveyPlainPipe`} render={({ field }) => (<FormItem><FormLabel>Plain Pipe (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.surveySlottedPipe`} render={({ field }) => (<FormItem><FormLabel>Slotted Pipe (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.outerCasingPipe`} render={({ field }) => (<FormItem><FormLabel>MS Casing (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.yieldDischarge`} render={({ field }) => (<FormItem><FormLabel>Discharge (LPH)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.zoneDetails`} render={({ field }) => (<FormItem><FormLabel>Zone Details (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.waterLevel`} render={({ field }) => (<FormItem><FormLabel>Water Level (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.drillingRemarks`} render={({ field }) => (<FormItem className="md:col-span-3"><FormLabel>Drilling Remarks</FormLabel><FormControl><Textarea {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage /></FormItem>)}/> </div> )}
                                                    {purpose === 'FPW' && ( <div className="grid md:grid-cols-3 gap-6"> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.diameter`} render={({ field }) => (<FormItem><FormLabel>Diameter (mm){isDiameterRequired && <span className="text-destructive">*</span>}</FormLabel><Select onValueChange={(value) => field.onChange(value === '_clear_' ? undefined : value)} value={field.value} disabled={!isSiteEditable}><FormControl><SelectTrigger><SelectValue placeholder="Select Diameter"/></SelectTrigger></FormControl><SelectContent><SelectItem value="_clear_">-- Clear Selection --</SelectItem>{siteDiameterOptions.map(o=><SelectItem key={o} value={o}>{o}</SelectItem>)}</SelectContent></Select><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.totalDepth`} render={({ field }) => (<FormItem><FormLabel>TD (m)</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.casingPipeUsed`} render={({ field }) => (<FormItem><FormLabel>Casing Pipe (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.yieldDischarge`} render={({ field }) => (<FormItem><FormLabel>Discharge (LPH)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.zoneDetails`} render={({ field }) => (<FormItem><FormLabel>Zone Details (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.waterLevel`} render={({ field }) => (<FormItem><FormLabel>Water Level (m)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/> <FormField control={form.control} name={`siteDetails.${absoluteIndex}.drillingRemarks`} render={({ field }) => (<FormItem className="md:col-span-3"><FormLabel>Drilling Remarks</FormLabel><FormControl><Textarea {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage /></FormItem>)}/> </div> )}
                                                </>
                                            )}
                                            ...
                                            {isDevPurpose && (
                                                <>
                                                    ...
                                                    <div className="grid md:grid-cols-3 gap-6">
                                                        ...
                                                        <FormField control={form.control} name={`siteDetails.${absoluteIndex}.totalDepth`} render={({ field }) => (<FormItem><FormLabel>TD (m)</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/>
                                                        <FormField control={form.control} name={`siteDetails.${absoluteIndex}.yieldDischarge`} render={({ field }) => (<FormItem><FormLabel>Discharge (LPH)</FormLabel><FormControl><Input {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/>
                                                        ...
                                                        <FormField control={form.control} name={`siteDetails.${absoluteIndex}.drillingRemarks`} render={({ field }) => (<FormItem className="md:col-span-2"><FormLabel>Developing Remarks</FormLabel><FormControl><Textarea {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/>
                                                    </div>
                                                </>
                                            )}
                                            ...
                                            {isMWSSSchemePurpose && (
                                                <>
                                                    ...
                                                    <div className="grid md:grid-cols-3 gap-6">
                                                        <FormField control={form.control} name={`siteDetails.${absoluteIndex}.yieldDischarge`} render={({ field }) => (<FormItem><FormLabel>Well Discharge (LPH)</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/>
                                                        <FormField control={form.control} name={`siteDetails.${absoluteIndex}.waterTankCapacity`} render={({ field }) => (<FormItem><FormLabel>Water Tank (L)</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/>
                                            ...
                                            <div className="grid md:grid-cols-3 gap-6">
                                                <FormField control={form.control} name={`siteDetails.${absoluteIndex}.dateOfCompletion`} render={({ field }) => ( 
                                                    <FormItem>
                                                ...
                                                <FormField control={form.control} name={`siteDetails.${absoluteIndex}.totalExpenditure`} render={({ field }) => (<FormItem><FormLabel>Expenditure (₹)</FormLabel><FormControl><Input type="text" inputMode="numeric" {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/>
                                                <FormField
                                                    control={form.control}
                                                    name={`siteDetails.${absoluteIndex}.workStatus`}
                                                    render={({ field }) => (
                                                    <FormItem>
                                                ...
                                                />
                                                <FormField control={form.control} name={`siteDetails.${absoluteIndex}.workRemarks`} render={({ field }) => (<FormItem className="md:col-span-3"><FormLabel>Work Remarks</FormLabel><FormControl><Textarea {...field} readOnly={!isSiteEditable} /></FormControl><FormMessage/></FormItem>)}/>
                                            </div>
                                        </div>
                                    </AccordionContent>
                                </AccordionItem>
                        ...
                        {totalSitePages > 1 && (
                            <div className="flex justify-center pt-2">
                                <PaginationControls
                                    ...
                                    onPageChange={setCurrentSitePage}
                                />
                            </div>
                        )}
