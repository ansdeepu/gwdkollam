                ...
                <ScrollArea className="h-full px-6 py-4">
                    <Form {...form}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <FormField name="latitude" control={control} render={({ field }) => <FormItem><FormLabel>Latitude</FormLabel><FormControl><Input type="number" step="any" {...field} onChange={e => field.onChange(e.target.value === '' ? undefined : +e.target.value)} readOnly={isReadOnly && !isSupervisor} /></FormControl><FormMessage /></FormItem>} />
                            <FormField name="longitude" control={control} render={({ field }) => <FormItem><FormLabel>Longitude</FormLabel><FormControl><Input type="number" step="any" {...field} onChange={e => field.onChange(e.target.value === '' ? undefined : +e.target.value)} readOnly={isReadOnly && !isSupervisor} /></FormControl><FormMessage /></FormItem>} />
                        </div>
                            </CardContent></Card>
                           ...
                           {isDrillingPurpose && (
                            ...
                            <Card><CardHeader><CardTitle>Work Implementation</CardTitle></CardHeader><CardContent>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                   ...
                                   <FormField name="tenderNo" control={control} render={({ field }) => <FormItem><FormLabel>Tender No.</FormLabel><FormControl><Input {...field} value={field.value || ''} readOnly={isReadOnly} /></FormControl><FormMessage/></FormItem>} />
                                   <FormField name="contractorName" control={control} render={({ field }) => <FormItem><FormLabel>Contractor</FormLabel><FormControl><Input {...field} value={field.value || ''} readOnly={isReadOnly} /></FormControl><FormMessage/></FormItem>} />
                                   <FormField name="supervisorUid" control={form.control} render={({ field }) => (<FormItem><FormLabel>Supervisor</FormLabel><Select onValueChange={(uid) => { const staff = supervisorList.find((s) => s.uid === uid); field.onChange(uid === '_clear_' ? undefined : uid); setValue("supervisorName", staff?.name || ""); setValue("supervisorDesignation", staff?.designation || undefined); }} value={field.value || ""} disabled={isReadOnly}><FormControl><SelectTrigger><SelectValue placeholder="Select Supervisor" /></SelectTrigger></FormControl><SelectContent><SelectItem value="_clear_" onSelect={(e) => { e.preventDefault(); field.onChange(undefined); setValue("supervisorName", ""); setValue("supervisorDesignation", undefined);}}>-- Clear Selection --</SelectItem>{supervisorList.map((s) => (<SelectItem key={s.uid} value={s.uid}>{s.name} ({s.designation})</SelectItem>))}</SelectContent></Select><FormMessage /></FormItem>)}/>
                                   <FormField control={form.control} name="workRemarks" render={({ field }) => ( <FormItem className="md:col-span-2"><FormLabel>Work Remarks</FormLabel><FormControl><Textarea {...field} value={field.value ?? ""} readOnly={isReadOnly} placeholder="Add any remarks for this site..."/></FormControl><FormMessage /></FormItem>)} />
                                </div>
                            </CardContent></Card>
                            )}
                                ...
                                <Card>
                                    ...
                                    <CardContent className="space-y-4">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <FormField name="totalDepth" control={control} render={({ field }) => <FormItem><FormLabel>Total Depth (m)</FormLabel><FormControl><Input type="number" {...field} onChange={e => field.onChange(e.target.value === '' ? undefined : +e.target.value)} readOnly={isReadOnly} /></FormControl><FormMessage /></FormItem>} />
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <FormField name="waterLevel" control={control} render={({ field }) => <FormItem className="md:col-span-1"><FormLabel>Static Water (m)</FormLabel><FormControl><Input {...field} value={field.value || ''} readOnly={isReadOnly} /></FormControl><FormMessage /></FormItem>} />
                                            <FormField name="developingRemarks" control={control} render={({ field }) => <FormItem className="md:col-span-2"><FormLabel>Remarks</FormLabel><FormControl><Textarea {...field} value={field.value || ''} readOnly={isReadOnly} /></FormControl><FormMessage /></FormItem>} />
                                        </div>
                                    </CardContent>
                                </Card>
                            )}
                               ...
                               <Card>
                                    <CardHeader><CardTitle>Drilling Details (Actuals)</CardTitle></CardHeader><CardContent className="space-y-4">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <FormField name="totalDepth" control={control} render={({field})=> <FormItem><FormLabel>Actual TD (m)</FormLabel><FormControl><Input type="number" {...field} onChange={e=>field.onChange(e.target.value === '' ? undefined : +e.target.value)} readOnly={isReadOnly && !isSupervisor} /></FormControl><FormMessage/></FormItem>} />
                                            ...
                                            <FormField name="yieldDischarge" control={control} render={({field})=> <FormItem><FormLabel>Yield (LPH)</FormLabel><FormControl><Input {...field} value={field.value || ''} readOnly={isReadOnly && !isSupervisor}/></FormControl><FormMessage/></FormItem>} />
                                            <FormField name="zoneDetails" control={control} render={({ field }) => <FormItem><FormLabel>Zone Details (m)</FormLabel><FormControl><Input {...field} value={field.value || ''} readOnly={isReadOnly && !isSupervisor} /></FormControl><FormMessage /></FormItem>} />
                                            <FormField name="waterLevel" control={control} render={({field})=> <FormItem><FormLabel>Static Water (m)</FormLabel><FormControl><Input {...field} value={field.value || ''} readOnly={isReadOnly && !isSupervisor}/></FormControl><FormMessage/></FormItem>} />
...
const ViewSiteDialog = ({ site, onCancel }: { site: SiteDetailFormData | null, onCancel: () => void }) => {
    ...
    const allDetails = {
        ...
        "Developing Details": isDevPurpose && {
            ...
            "Water Level (m)": site.waterLevel, "Remarks": site.developingRemarks,
        },
...
export default function DataEntryFormComponent({ fileNoToEdit, initialData, supervisorList, userRole, workTypeContext, pageToReturnTo, isFormDisabled = false }: DataEntryFormProps) {
  ...
  const isReadOnly = (fieldName?: keyof SiteDetailFormData) => {
    ...
    if (isSupervisor) {
      ...
      const supervisorEditableFields: (keyof SiteDetailFormData)[] = ['workStatus', 'dateOfCompletion', 'drillingRemarks', 'developingRemarks', 'workRemarks', 'totalDepth', 'yieldDischarge', 'waterLevel', 'zoneDetails', 'latitude', 'longitude'];
      ...
      <form onSubmit={handleSubmit(onSubmit, onInvalid)} className="space-y-6">
        ...
        <Card>
            <CardHeader className="flex flex-row justify-between items-start">
                ...
                {isEditor && !isFormDisabled && <Button type="button" onClick={() => openDialog('application', getValues())}><Edit className="h-4 w-4 mr-2" />Edit</Button>}
            </CardHeader>
            <CardContent>
        ...
        <Card>
            <CardHeader className="flex flex-row justify-between items-start">
                ...
                {isEditor && !isFormDisabled && <Button type="button" onClick={() => openDialog('remittance', createDefaultRemittanceDetail())}><PlusCircle className="h-4 w-4 mr-2" />Add</Button>}
            </CardHeader>
            <CardContent>
                        ...
                        <TableBody>
                            {remittanceFields.length > 0 ? remittanceFields.map((item, index) => (
                                <TableRow key={item.id}>
                                    ...
                                    {isEditor && !isFormDisabled && <TableCell><div className="flex gap-1"><Button type="button" variant="ghost" size="icon" onClick={() => openDialog('remittance', { index, ...item })}><Edit className="h-4 w-4"/></Button><Button type="button" variant="ghost" size="icon" className="text-destructive" onClick={() => setItemToDelete({type: 'remittance', index})}><Trash2 className="h-4 w-4"/></Button></div></TableCell>}
                                </TableRow>
        ...
        <Card>
            <CardHeader className="flex flex-row justify-between items-start">
                ...
                {isEditor && !isFormDisabled && <Button type="button" onClick={() => openDialog('site', {})}><PlusCircle className="h-4 w-4 mr-2" />Add Site</Button>}
            </CardHeader>
            <CardContent>
                                    ...
                                    <div className="flex justify-between items-center w-full">
                                        ...
                                        <div className="flex items-center space-x-1 mr-2">
                                            <TooltipProvider>
                                                ...
                                                {!isViewer && !isFormDisabled && !isSupervisor && (
                                                    <>
                                                        <Tooltip>
                                                            <TooltipTrigger asChild>
        ...
        <Card>
            <CardHeader className="flex flex-row justify-between items-start">
                <div><CardTitle className="text-xl">4. Payment Details</CardTitle></div>
                 {isEditor && !isFormDisabled && <Button type="button" onClick={() => openDialog('payment', createDefaultPaymentDetail())}><PlusCircle className="h-4 w-4 mr-2" />Add</Button>}
            </CardHeader>
            <CardContent>
                 <div className="relative max-h-[400px] overflow-auto">
                    <Table>
                        ...
                        <TableBody>
                            {paymentFields.length > 0 ? paymentFields.map((item, index) => (
                                <TableRow key={item.id}>
                                    ...
                                    {isEditor && !isFormDisabled && <TableCell className="p-1"><div className="flex"><Button type="button" variant="ghost" size="icon" onClick={() => openDialog('payment', { index, ...item })}><Edit className="h-3 w-3"/></Button><Button type="button" variant="ghost" size="icon" className="text-destructive" onClick={() => setItemToDelete({type: 'payment', index})}><Trash2 className="h-3 w-3"/></Button></div></TableCell>}
                                </TableRow>
