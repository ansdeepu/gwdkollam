...
export default function ETenderNoticeBoard() {
  ...
  const categorizedTenders = useMemo(() => {
    ...
    activeTenders.forEach(t => {
        ...
        // To be opened: Before opening date, and bid hasn't been officially opened
        if (opening && isValid(opening) && isBefore(now, opening) && !t.dateOfOpeningBid) {
            toBeOpened.push(t);
        }
                ...
                <TabsContent key={cat.type} value={cat.type} className="h-full m-0">
                  <ScrollArea className="h-full pr-3">
                     {renderTenderList(
                          ...
                          cat.type === 'toBeOpened' ? (t) => `Opens: ${formatDateSafe(t.dateTimeOfOpening, true)}` : undefined
                      )}
                  </ScrollArea>
                </TabsContent>
              ))}
